...
def start_redis():
    ...
    try:
        ...
        if return_code:
            log.error("Failed to start redis: return code: " + str(return_code) + " Error: " + process_info.stdout.decode(
                    "utf-8"))
            sys.exit(1)
        elif return_code == 0:
...
def start_rabbitmq():
    ...
    return_code = process_info.returncode
    if return_code:
        log.error("Failed to start rabbitmq-server: return code: " + str(return_code) + " Error: " + process_info.stdout.decode("utf-8"))
        sys.exit(1)
    elif return_code == 0:
...
def start_mongo():
    ...
    return_code = process_info.returncode
    if return_code:
        log.error("Failed to start MongoDb: return code: " + str(return_code) + " Error: " + process_info.stdout.decode("utf-8"))
        sys.exit(1)
    elif return_code == 0:
